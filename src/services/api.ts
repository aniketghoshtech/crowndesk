...
async function handleResponse<T = any>(res: Response, defaultError = 'Request failed'): Promise<T> {
  ...
  if (!res.ok) {
    let errorMsg = data?.error || data?.message || (typeof data === 'string' ? data : `${defaultError} (Status ${res.status})`);
    if (typeof errorMsg === 'string' && errorMsg.includes('FUNCTION_INVOCATION_FAILED')) {
      errorMsg = 'Server backend is initializing or encountered a transient connection issue. Please retry.';
    }
